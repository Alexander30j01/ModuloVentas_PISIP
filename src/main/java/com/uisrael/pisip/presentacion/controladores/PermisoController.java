...
public class PermisoController {
	...
	public PermisoResponseDto registrar(@Valid @RequestBody PermisoRequestDto request) {
		return mapper.toResponse(permisoUseCase.registrar(mapper.toDomain(request)));
	}

	@GetMapping("/{idPermiso}")
	public PermisoResponseDto buscarPorId(@PathVariable int idPermiso) {
		return mapper.toResponse(permisoUseCase.buscarPorId(idPermiso));
	}

	@GetMapping
	public List<PermisoResponseDto> listarTodos() {
		return permisoUseCase.listarTodos().stream().map(mapper::toResponse).toList();
	}
